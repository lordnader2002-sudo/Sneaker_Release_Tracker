# file: fetch_release_footlocker.py
...
SOURCE_URL = "https://www.footlocker.com/release-calendar.html"
SOURCE_NAME = "footlocker"

# Matches "Mar 7" or "March 7" optionally with year
...
def extract_rows(soup: BeautifulSoup) -> list[dict[str, Any]]:
    ...
    # Heuristic: scan links/cards; look for date text near title text
    for a in soup.find_all("a", href=True):
        title = normalize_text(a.get_text(" ", strip=True))
        if len(title) < 8:
            continue

        # search nearby text for a date
        container = a.parent
        blob = ""
        for _ in range(4):
        ...
        # Build a date string that parse_date_flexible can understand
        month = m.group(1)
        day = m.group(2)
        year = m.group(3)
        ...
        href = a["href"]
        if href.startswith("/"):
            href = "https://www.footlocker.com" + href
